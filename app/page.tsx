...
const translations = {
  en: {
    ...
    // Footer
    redefining: "Redefining the acquisition of pre-owned and high-performance vehicles.",
    ...
    waText: "Hello, I'm interested in the",
    // Natural language search
    nlLabel: "Smart Search",
    nlPlaceholder: "E.g.: white BMW automatic under $30,000 or SUV under 50,000 miles...",
    nlBtn: "Search",
    nlClear: "Clear search",
    nlNoResults: "No vehicles matched your search. Try different keywords.",
  },
  es: {
    ...
    // Footer
    redefining: "Redefiniendo la adquisición de vehículos pre-owned y de alto rendimiento.",
    ...
    waText: "Hola, estoy interesado en el",
    // Natural language search
    nlLabel: "Búsqueda Inteligente",
    nlPlaceholder: "Ej: BMW blanco automático menos de $30,000 o SUV menos de 50,000 millas...",
    nlBtn: "Buscar",
    nlClear: "Limpiar búsqueda",
    nlNoResults: "Ningún vehículo coincide con tu búsqueda. Intenta con otras palabras.",
  }
};
...
export default function HomePage() {
  ...
  // Función envuelta en useCallback para estabilidad de rendimiento
  ...
  const [pointerStart, setPointerStart] = useState<number | null>(null); // Para deslizar testimonios
  ...
  const fetchPublicTestimonials = useCallback(async () => {
    ...
    if (data) setPublicTestimonials(data);
  }, []);

  const fetchAdminTestimonials = useCallback(async () => {
    const { data } = await supabase.from('testimonials').select('*').order('created_at', { ascending: false });
    if (data) setAdminTestimonials(data);
  }, []);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (publicTestimonials.length > 0) {
        setCurrentTestimonial((prev) => (prev + 1) % publicTestimonials.length);
      }
    }, 8000); // Cambiado a 8 segundos para que sea más lento
  ...
  // Cargar datos del admin al loguearse o cambiar de pestaña
  useEffect(() => {
    if (isAuthenticated) {
      fetchLeads();
      fetchAdminTestimonials();
    }
  }, [isAuthenticated, fetchLeads, fetchAdminTestimonials, adminTab]);

  const handleReviewSubmit = async () => {
    if (!reviewForm.comment.trim()) {
      alert(lang === 'es' ? "Por favor escribe tu experiencia." : "Please write your experience.");
      return;
    }
  ...
  const handleDecodeVIN = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Fallo en la comunicación con NHTSA");
      ...
      alert("Hubo un problema de conexión con la base de datos de NHTSA.");
    } finally {
      setIsDecoding(false);
    }
  };
  ...
  const rotateImage = async (index: number) => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        img.onload = resolve;
        img.onerror = () => reject(new Error("CORS bloqué la imagen"));
      });
      const canvas = document.createElement("canvas");
      ...
      alert("Hubo un error al rotar la imagen. Verifica las políticas CORS en Supabase.");
    } finally {
  ...
  // Helper para extraer la imagen principal de forma segura
  ...
  const renderCarCard = (car: any, isHighlighted: boolean = false) => {
    ...
    const primaryImgUrl = getPrimaryImage(car);
  ...
  if (isStaffMode) {
    ...
    return (
      <main className="bg-[#111C2D] text-white min-h-screen font-sans flex flex-col md:flex-row overflow-hidden">
        <aside className="w-full md:w-64 bg-[#0B1320] border-b md:border-r border-white/5 flex flex-col h-auto md:h-screen shrink-0">
          <div className="p-4 md:p-6 border-b border-white/5 flex md:flex-col justify-between items-center md:items-start gap-4">
            <div className="flex items-center gap-3 md:mb-6"><img src="/logo-vertical.svg" alt="Logo" className="h-6 md:h-8" /><span className="hidden md:inline text-base font-bold tracking-widest uppercase text-sierra-gold">Sierra Apex</span></div>
            <div className="flex items-center gap-3"><div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#162439] flex items-center justify-center text-sierra-gold border border-white/10 font-bold text-xs md:text-sm">RH</div><div className="hidden md:block"><p className="text-sm font-bold">Reiniel Hernandez</p><p className="text-xs text-white/50 uppercase tracking-widest">Administrador</p></div></div>
          </div>
          <nav className="flex-1 flex flex-row overflow-x-auto md:flex-col p-2 md:p-4 space-x-2 md:space-x-0 md:space-y-2 hide-scrollbar">
            <button onClick={() => { setAdminTab('inventory'); setEditingVehicleId(null); }} className={`shrink-0 md:w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${adminTab === 'inventory' || adminTab === 'add-vehicle' ? 'bg-sierra-gold text-black' : 'text-white/60 hover:bg-[#162439] hover:text-white'}`}>Inventario</button>
            <button onClick={() => setAdminTab('leads')} className={`shrink-0 md:w-full flex items-center justify-between px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${adminTab === 'leads' ? 'bg-sierra-gold text-black' : 'text-white/60 hover:bg-[#162439] hover:text-white'}`}>
              <div className="flex items-center gap-3">Leads & CRM</div>
              {(leads || []).filter((l: any) => l?.status === 'Nuevo').length > 0 && <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full font-bold ml-2">{(leads || []).filter((l: any) => l?.status === 'Nuevo').length}</span>}
            </button>
            <button onClick={() => setAdminTab('testimonials')} className={`shrink-0 md:w-full flex items-center justify-between px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${adminTab === 'testimonials' ? 'bg-sierra-gold text-black' : 'text-white/60 hover:bg-[#162439] hover:text-white'}`}>
              <div className="flex items-center gap-3">Opiniones</div>
              {(adminTestimonials || []).filter((t: any) => t?.status === 'Pendiente').length > 0 && <span className="bg-sierra-gold text-black text-xs px-2 py-0.5 rounded-full font-bold ml-2">{(adminTestimonials || []).filter((t: any) => t?.status === 'Pendiente').length}</span>}
            </button>
          </nav>
          <div className="p-2 md:p-4 border-t border-white/5"><button onClick={handleLogout} className="w-full px-4 py-3 border border-white/10 hover:bg-white/5 rounded-xl text-xs font-bold uppercase tracking-widest text-white/50 hover:text-white transition-all">Cerrar Sesión</button></div>
        </aside>

        <section className="flex-1 h-screen overflow-y-auto pb-20 md:pb-0">
          <header className="bg-[#111C2D]/80 backdrop-blur-md sticky top-0 z-10 border-b border-white/5 px-4 md:px-8 py-4 flex justify-between items-center">
            <h2 className="text-sm md:text-xl font-light tracking-widest uppercase truncate">
              {adminTab === 'inventory' ? 'Inventario Existente' : adminTab === 'leads' ? 'Gestión de Prospectos' : adminTab === 'testimonials' ? 'Revisión de Opiniones' : (editingVehicleId ? 'Editar Vehículo' : 'Añadir Nuevo Vehículo')}
            </h2>
            <div className="flex gap-4">{adminTab === 'inventory' && (<button onClick={() => { resetForm(); setAdminTab('add-vehicle'); }} className="px-4 md:px-6 py-2 bg-sierra-gold text-black rounded-lg text-[10px] md:text-xs font-bold uppercase tracking-widest hover:bg-white transition-colors shadow-lg shrink-0">+ Añadir</button>)}</div>
          </header>

          <div className="p-4 md:p-8 max-w-6xl mx-auto">
            {adminTab === 'add-vehicle' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-[#162439] p-4 md:p-8 rounded-2xl border border-white/5 shadow-xl mb-8">
                  <h3 className="text-base font-bold text-sierra-gold uppercase tracking-widest mb-6">¿Qué estás vendiendo?</h3>
                  
                  <div className="mb-10 p-4 md:p-8 bg-[#111C2D] border border-sierra-gold/30 rounded-xl relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                      <input type="text" placeholder="INGRESA EL VIN AQUÍ..." value={vinToDecode} onChange={(e) => setVinToDecode(e.target.value.toUpperCase())} maxLength={17} className="flex-1 bg-white/5 border border-white/10 rounded-xl p-4 outline-none focus:border-sierra-gold text-base md:text-xl tracking-[0.2em] font-mono uppercase text-white placeholder:text-white/30 transition-all" />
                      <button onClick={handleDecodeVIN} disabled={isDecoding || vinToDecode.length !== 17} className={`w-full md:w-48 py-4 font-bold rounded-xl uppercase text-sm tracking-widest transition-all ${isDecoding || vinToDecode.length !== 17 ? 'bg-white/5 text-white/40 cursor-not-allowed border border-white/10' : 'bg-sierra-gold text-black hover:bg-white shadow-[0_0_20px_rgba(212,175,55,0.3)]'}`}>{isDecoding ? 'Decodificando...' : 'Decodificar VIN'}</button>
                    </div>
                  </div>
                  ...
                  <div className={`transition-all duration-700 ${decodedData ? 'opacity-100' : 'opacity-40'}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8 border-t border-white/5 pt-6">
                      <div>
                    ...
                    {decodedData && newCarForm.make && newCarForm.model && newCarForm.year && (
                      <div className="mb-8 p-4 bg-[#111C2D] border border-white/10 rounded-xl">
                        <p className="text-xs uppercase text-white/40 tracking-widest mb-3 font-bold">Ver precios del mercado</p>
                        <div className="flex flex-wrap gap-3">
                          <a
                            href={`https://www.autotrader.com/cars-for-sale/used-cars/${encodeURIComponent(newCarForm.make.toLowerCase().replace(/\s+/g, '-'))}/${encodeURIComponent(newCarForm.model.toLowerCase().replace(/\s+/g, '-'))}/?startYear=${newCarForm.year}&endYear=${newCarForm.year}${newCarForm.miles ? `&maxMileage=${newCarForm.miles}` : ''}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 px-4 py-2 border border-white/10 rounded-lg text-xs font-bold uppercase tracking-widest text-white/70 hover:border-sierra-gold hover:text-sierra-gold transition-all"
                          >
                            AutoTrader ↗
                          </a>
                          <a
                            href={`https://www.cars.com/shopping/results/?make_slug=${encodeURIComponent(newCarForm.make.toLowerCase().replace(/\s+/g, '_'))}&model_slug=${encodeURIComponent(newCarForm.model.toLowerCase().replace(/\s+/g, '_'))}&year_min=${newCarForm.year}&year_max=${newCarForm.year}&maximum_distance=75&stock_type=used&sort=list_price_asc${newCarForm.miles ? `&mileage_max=${newCarForm.miles}` : ''}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 px-4 py-2 border border-white/10 rounded-lg text-xs font-bold uppercase tracking-widest text-white/70 hover:border-sierra-gold hover:text-sierra-gold transition-all"
                          >
                        ...
                        {uploadedImages.map((url: any, idx: number) => (
                          ...
                          >
                            ...
                            <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-sm">
                              <button onClick={(e) => { e.preventDefault(); rotateImage(idx); }} title="Girar 90°" className="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-white hover:bg-sierra-gold hover:text-black transition-colors"><svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg></button>
                              <button onClick={(e) => { e.preventDefault(); deleteImage(idx); }} title="Eliminar Foto" className="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-red-400 hover:bg-red-500 hover:text-white transition-colors">✕</button>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-end items-center gap-4 border-t border-white/5 pt-8 mt-8">
                    <button onClick={resetForm} className="w-full sm:w-auto px-8 py-4 border border-white/10 hover:bg-white/5 rounded-xl text-xs uppercase font-bold tracking-widest transition-all">Cancelar</button>
                    {/* Input file restringido a formatos web estándar para evitar archivos .heic que rompen en Windows/Android */}
                    ...
                    <button onClick={triggerFileInput} disabled={isUploading} className={`w-full sm:w-auto px-8 py-4 border rounded-xl text-xs uppercase tracking-widest font-bold transition-all ${isUploading ? 'border-white/10 text-white/40 cursor-wait' : 'border-sierra-gold text-sierra-gold hover:bg-sierra-gold hover:text-black'}`}>
                      {uploadedImages.length > 0 ? 'Añadir más fotos' : 'Subir Fotos'}
                    </button>
                    <button onClick={handleSaveVehicle} disabled={isUploading} className={`w-full sm:w-auto px-8 py-4 font-bold rounded-xl text-xs uppercase tracking-widest transition-all ${decodedData && !isUploading ? 'bg-sierra-gold text-black hover:bg-white shadow-[0_0_15px_rgba(212,175,55,0.2)]' : 'bg-white/10 text-white/40 cursor-not-allowed'}`}>
            ...
            {adminTab === 'inventory' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-[#162439] p-4 md:p-6 rounded-2xl border border-white/5 flex flex-col md:flex-row gap-4 md:items-end">
                  <div className="flex flex-col w-full md:w-auto"><label className="text-xs text-white/50 font-bold uppercase tracking-widest mb-2 ml-1">Stock #</label><input type="text" placeholder="Ej: SA-001" className="bg-[#111C2D] border border-white/10 rounded-lg p-3 text-sm outline-none focus:border-sierra-gold w-full md:w-40" /></div>
                  <div className="flex flex-col w-full md:w-auto"><label className="text-xs text-white/50 font-bold uppercase tracking-widest mb-2 ml-1">Ordenar Por</label><select className="bg-[#111C2D] border border-white/10 rounded-lg p-3 text-sm outline-none focus:border-sierra-gold w-full md:w-48 cursor-pointer"><option>Agregados Recientemente</option><option>Precio: Mayor a Menor</option></select></div>
                </div>

                <div className="space-y-4">
                  {isLoading ? (
                    <div className="p-10 md:p-20 text-center text-white/50 uppercase tracking-widest text-xs md:text-sm">Cargando base de datos...</div>
                  ) : inventory.length === 0 ? (
                    <div className="p-10 md:p-20 text-center bg-[#162439] rounded-2xl border border-white/5">
                      <p className="text-white/50 uppercase tracking-widest text-xs md:text-sm mb-4">No hay vehículos en el inventario.</p>
                      ...
                      <div key={car.id} className={`bg-[#162439] rounded-2xl border overflow-hidden flex flex-col md:flex-row shadow-lg hover:border-sierra-gold/30 transition-all border-white/5`}>
                        ...
                        <div className="flex-1 p-4 md:p-6 flex flex-col justify-between">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 border-t border-white/5 pt-4">
                            <div><p className="text-[10px] md:text-xs text-white/40 uppercase tracking-widest mb-1">Millaje</p><p className="text-xs md:text-sm font-bold">{car?.miles || 0}</p></div>
                            <div><p className="text-[10px] md:text-xs text-white/40 uppercase tracking-widest mb-1">Color Ext.</p><p className="text-xs md:text-sm font-bold text-white truncate">{car?.color || 'N/A'}</p></div>
                            <div><p className="text-[10px] md:text-xs text-white/40 uppercase tracking-widest mb-1">Color Int.</p><p className="text-xs md:text-sm font-bold text-white truncate">{car?.interior_color || 'N/A'}</p></div>
                            <div><p className="text-[10px] md:text-xs text-white/40 uppercase tracking-widest mb-1">Días Web</p><p className="text-xs md:text-sm font-bold">{car?.days_on_market || 0}</p></div>
                          </div>
                        </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {(leads || []).length === 0 ? (
                  ...
                  <div className="grid gap-4">
                    {(leads || []).map((lead: any) => (
                      <div key={lead.id} className={`bg-[#162439] p-4 md:p-6 rounded-2xl border transition-all shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-6 ${lead?.status === 'Nuevo' ? 'border-sierra-gold/30' : 'border-white/5'}`}>
                        <div className="flex gap-4 md:gap-6 items-start md:items-center w-full md:w-auto flex-col md:flex-row">
                          <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-[#111C2D] border border-white/10 flex items-center justify-center text-sierra-gold font-bold text-lg md:text-xl uppercase shrink-0">
                            {lead?.name ? lead.name.substring(0,2) : 'CA'}
                          </div>
                          <div className="flex-1 w-full">
                            <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 mb-2">
                              <h4 className="text-lg md:text-xl font-bold">{lead?.name || 'Cliente Anónimo'}</h4>
                              <span className={`self-start md:self-auto text-[10px] md:text-xs uppercase tracking-widest font-bold px-3 py-1 rounded-full ${lead?.status === 'Nuevo' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-green-500/20 text-green-400 border border-green-500/30'}`}>
                                {lead?.status || 'N/A'}
                              </span>
                            </div>
                            <p className="text-xs md:text-sm text-white/80 font-mono mb-2 break-all">
                              {lead?.email || ''} {(lead?.email && lead?.phone) ? ' • ' : ''} {lead?.phone || ''}
                            </p>
                            <p className="text-xs md:text-sm text-white/50 mb-3">Interés: <span className="text-white font-bold">{lead?.car_title || 'N/A'}</span></p>
                            {lead?.message && (
                              <div className="bg-[#111C2D] p-3 md:p-4 rounded-xl border border-white/5 mt-2">
                                <p className="text-xs md:text-sm text-white/70 italic">"{lead.message}"</p>
                              </div>
                            )}
                          </div>
                        </div>
                        <div className="text-left md:text-right flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto gap-4 border-t border-white/5 md:border-none pt-4 md:pt-0">
                          <p className="text-[10px] md:text-xs text-white/40 uppercase tracking-widest font-bold">{lead?.created_at ? new Date(lead.created_at).toLocaleDateString() : ''}</p>
                          {lead?.status === 'Nuevo' && (
            ...
            {/* --- PANEL DE REVISIÓN DE OPINIONES --- */}
            {adminTab === 'testimonials' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {(adminTestimonials || []).length === 0 ? (
                  <div className="p-10 text-center bg-[#162439] rounded-2xl border border-white/5">
                    <p className="text-white/50 uppercase tracking-widest text-sm">No hay opiniones registradas.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(adminTestimonials || []).map((testimonio: any) => (
                      <div key={testimonio.id} className={`bg-[#162439] p-6 rounded-2xl border flex flex-col justify-between shadow-lg transition-all ${testimonio.status === 'Pendiente' ? 'border-sierra-gold/50' : testimonio.status === 'Aprobado' ? 'border-green-500/30 opacity-70' : 'border-red-500/30 opacity-50'}`}>
                        <div>
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              <h4 className="text-lg font-bold text-white">{testimonio.name || 'Anónimo'}</h4>
                              <p className="text-xs text-white/40 uppercase tracking-widest mt-1 font-bold">{testimonio?.created_at ? new Date(testimonio.created_at).toLocaleDateString() : ''}</p>
                            </div>
                            <span className={`text-[10px] uppercase tracking-widest font-bold px-3 py-1 rounded-full ${testimonio.status === 'Pendiente' ? 'bg-sierra-gold/20 text-sierra-gold border border-sierra-gold/30' : testimonio.status === 'Aprobado' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
                              {testimonio.status}
                            </span>
                          </div>
                          <div className="flex text-sierra-gold mb-4">{[...Array(testimonio.rating || 5)].map((_, i) => <span key={i}>★</span>)}</div>
                          <p className="text-sm text-white/80 italic leading-relaxed mb-6">"{testimonio.comment}"</p>
                        </div>
                        
                        <div className="flex gap-2 md:gap-3 border-t border-white/5 pt-4">
                          {testimonio.status !== 'Aprobado' && (
                            <button onClick={() => updateTestimonialStatus(testimonio.id, 'Aprobado')} className="flex-1 py-2 bg-green-500/10 text-green-400 hover:bg-green-500 hover:text-white border border-green-500/30 rounded-lg text-[10px] md:text-xs uppercase tracking-widest font-bold transition-all">Aprobar</button>
                          )}
                          {testimonio.status !== 'Rechazado' && (
                            <button onClick={() => updateTestimonialStatus(testimonio.id, 'Rechazado')} className="flex-1 py-2 bg-yellow-500/10 text-yellow-400 hover:bg-yellow-500 hover:text-white border border-yellow-500/30 rounded-lg text-[10px] md:text-xs uppercase tracking-widest font-bold transition-all">Rechazar</button>
                          )}
                          <button onClick={() => handleDeleteTestimonial(testimonio.id)} className="flex-1 py-2 bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white border border-red-500/30 rounded-lg text-[10px] md:text-xs uppercase tracking-widest font-bold transition-all">Borrar</button>
                        </div>
                      </div>
                    ))}
                  </div>
  ...
  return (
    <main className="bg-[#111C2D] text-white font-sans min-h-screen relative">
      <header className="fixed top-0 left-0 w-full z-[60] bg-[#111C2D]/70 backdrop-blur-xl border-b border-white/5 px-4 md:px-6 h-16 flex justify-between items-center">
        <div className="absolute left-4 md:left-6 top-1/2 md:top-[62%] -translate-y-1/2 z-[61] cursor-pointer" onClick={() => scrollToSection('top')}>
          {/* Logo para celular (Vertical) */}
          <img src="/logo-vertical.svg" alt="Sierra Apex Group" className="block md:hidden h-[60px] object-contain drop-shadow-lg" />
          {/* Logo para PC (Horizontal original) */}
          <img src="/logo.svg" alt="Sierra Apex Group" className="hidden md:block h-[200px] object-contain drop-shadow-2xl" onError={(e) => { e.currentTarget.style.display = 'none'; document.getElementById('fallback-logo')!.style.display = 'flex'; }} />
          <div id="fallback-logo" className="hidden text-sm md:text-xl font-bold tracking-widest uppercase items-center gap-1 md:gap-2"><span className="text-white">SIERRA</span><span className="text-sierra-gold">APEX</span></div>
        </div>
        <div className="flex-1"></div>
        <div className="flex gap-3 md:gap-6 items-center text-[10px] md:text-sm uppercase tracking-widest text-white/60 relative z-[60] font-bold">
          <span onClick={() => scrollToSection('inventario')} className="hover:text-white cursor-pointer transition-colors">{t.navInventory}</span>
          <span onClick={() => scrollToSection('testimonios')} className="hidden sm:inline hover:text-white cursor-pointer transition-colors">{t.navReviews}</span>
          
          {/* BOTÓN IDIOMA */}
          <button onClick={() => setLang(lang === 'en' ? 'es' : 'en')} className="flex items-center gap-1 bg-white/5 hover:bg-white/10 px-2 py-1 md:px-3 md:py-1.5 rounded transition-colors text-white">
          ...
          <button onClick={() => setIsStaffMode(true)} className="px-2 py-1 md:px-4 md:py-2 border border-white/10 hover:border-sierra-gold hover:text-white rounded transition-all">{t.navStaff}</button>
        </div>
      </header>
      ...
      <section className="relative w-full h-screen flex flex-col justify-between">
        ...
        <div className="relative z-10 text-center px-4 pt-16">
          <h1 className="text-4xl md:text-8xl font-bold tracking-tighter mb-8 md:mb-12 uppercase drop-shadow-2xl">{t.heroTitle}</h1>
          <button onClick={() => scrollToSection('inventario')} className="px-8 py-4 md:px-10 md:py-5 border border-white/40 text-white hover:bg-sierra-gold hover:text-black hover:border-sierra-gold transition-all duration-500 rounded-full uppercase tracking-[0.2em] text-[10px] md:text-sm font-bold backdrop-blur-sm shadow-2xl">{t.heroBtn}</button>
        </div>
        <div className="relative z-10 flex-1 flex items-end justify-center pb-8 px-6">
          <div className="max-w-3xl text-center">
            <h2 className="text-[10px] md:text-xs tracking-[0.4em] text-sierra-gold mb-4 uppercase font-bold drop-shadow-md">{t.standardTitle}</h2>
            <p className="text-lg md:text-2xl font-light leading-relaxed text-white/90 italic">{t.standardDesc}</p>
          </div>
        </div>
      </section>

      {/* --- SECCIÓN INTEGRADA DE INVENTARIO Y BUSCADOR --- */}
      <section id="inventario" className="pt-16 pb-12 px-4 md:px-6 bg-[#111C2D] min-h-screen">
        <div className="max-w-7xl mx-auto w-full">
          
          <div className="flex flex-col md:flex-row justify-between md:items-end mb-4 border-b border-white/5 pb-2 gap-4">
            <h3 className="text-2xl md:text-4xl font-light tracking-tight text-white uppercase text-center md:text-left">{t.collectionTitle1} <span className="font-bold text-sierra-gold">{t.collectionTitle2}</span></h3>
            <div className="flex items-center justify-center md:justify-end gap-4">
              {searchActive && (
                <button onClick={handleClearSearch} className="text-[10px] md:text-xs text-sierra-gold hover:text-black uppercase tracking-widest border border-sierra-gold bg-sierra-gold/20 hover:bg-sierra-gold px-4 py-2 font-bold rounded-lg transition-all shadow-[0_0_10px_rgba(212,175,55,0.3)]">
                  {t.viewAll}
                </button>
              )}
          ...
          <div className="mb-8 bg-[#111C2D]/60 p-4 md:p-8 rounded-2xl backdrop-blur-3xl border border-white/10 shadow-lg relative z-20">
            <div className="mb-4 text-center md:text-left"><h4 className="text-[10px] md:text-xs text-sierra-gold uppercase tracking-[0.4em] font-bold">{t.advSearch}</h4></div>
            <div className="flex flex-col md:flex-row gap-6 md:gap-8 justify-between items-end w-full">
              <div className="flex flex-col md:flex-row gap-4 md:gap-8 w-full md:w-auto">
                <div className="flex flex-col w-full md:w-auto">
                  <label className="text-[10px] md:text-xs text-white/60 font-bold uppercase tracking-[0.2em] mb-2 ml-1">{t.make}</label>
                  <select value={selectedBrand} onChange={(e) => { setSelectedBrand(e.target.value); setSelectedModel(''); setSearchActive(true); }} className="bg-transparent border-b border-white/20 p-2 text-sm md:text-base focus:border-sierra-gold outline-none text-white cursor-pointer w-full md:w-40 transition-all">
                ...
                <div className="flex flex-col w-full md:w-auto">
                  <label className="text-[10px] md:text-xs text-white/60 font-bold uppercase tracking-[0.2em] mb-2 ml-1">{t.model}</label>
                  <select disabled={!selectedBrand} value={selectedModel} onChange={(e) => { setSelectedModel(e.target.value); setSearchActive(true); }} className={`bg-transparent border-b p-2 text-sm md:text-base outline-none text-white w-full md:w-40 transition-all ${!selectedBrand ? 'border-white/5 text-white/20 cursor-not-allowed' : 'border-white/20 focus:border-sierra-gold cursor-pointer'}`}>
                ...
                <div className="flex flex-col w-full md:w-auto mt-2 md:mt-0">
                  <span className="text-[10px] md:text-xs text-white/80 uppercase tracking-widest mb-3 ml-1">{t.maxBudget}: <span className="text-sierra-gold font-bold">${(currentPrice || 0).toLocaleString()}</span></span>
                  <div className="flex items-center gap-2 md:gap-4">
                    <span className="text-[10px] md:text-xs text-white/40 font-bold">${(minInventoryPrice || 0).toLocaleString()}</span>
                    <input type="range" min={minInventoryPrice} max={maxInventoryPrice} value={currentPrice} onChange={(e) => { setCurrentPrice(Number(e.target.value)); setSearchActive(true); }} className="accent-sierra-gold cursor-pointer w-full md:w-48" />
                    <span className="text-[10px] md:text-xs text-white/40 font-bold">${(maxInventoryPrice || 0).toLocaleString()}</span>
                  </div>
                </div>
              </div>
              <button onClick={handleSearch} className="w-full md:w-auto px-10 py-3 md:py-4 bg-sierra-gold text-black font-bold rounded-xl hover:bg-white transition-all duration-500 uppercase tracking-widest text-[10px] md:text-xs shadow-lg mt-4 md:mt-0">{t.searchBtn}</button>
            </div>
          </div>

          {isLoading ? (
            <div className="p-10 md:p-20 text-center text-white/50 uppercase font-bold tracking-widest text-xs md:text-sm">{t.loading}</div>
          ) : inventory.length === 0 ? (
            <div className="p-10 md:p-20 text-center bg-white/5 rounded-2xl border border-white/5">
              <p className="text-white/50 uppercase font-bold tracking-widest text-xs md:text-sm">{t.preparing}</p>
            </div>
          ) : nlResults !== null ? (
            ...
            <div className="animate-in fade-in duration-700">
              {exactMatches.length > 0 ? (
                <div className="mb-16">
                  <h4 className="text-sierra-gold font-bold uppercase tracking-widest mb-6 flex items-center gap-3 text-xs md:text-sm">
                    <span className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-sierra-gold animate-pulse"></span> {t.exactMatches}
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                <div className="mb-16 bg-[#162439] border border-white/5 p-6 md:p-10 rounded-2xl text-center shadow-lg">
                  <h4 className="text-white font-bold uppercase tracking-widest mb-3 text-sm md:text-lg">{t.noMatchesTitle}</h4>
                  <p className="text-white/60 text-sm md:text-base">{t.noMatchesDesc}</p>
                </div>
              )}

              {otherCars.length > 0 && (
                <div>
                  <h4 className="text-white/60 font-bold uppercase tracking-widest mb-6 flex items-center gap-2 text-xs md:text-sm">{t.exploreOthers}</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 opacity-80 hover:opacity-100 transition-all duration-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 animate-in fade-in">
      ...
      {/* --- SECCIÓN TESTIMONIOS --- */}
      <section id="testimonios" className="py-16 md:py-24 px-4 md:px-6 bg-[#131F33] border-y border-white/5 overflow-hidden">
        <div className="max-w-7xl mx-auto text-center">
          <h3 className="text-[10px] md:text-sm font-bold text-sierra-gold uppercase tracking-[0.4em] mb-4">{t.experiencesTitle}</h3>
          <h2 className="text-3xl md:text-5xl font-bold mb-10 md:mb-16 uppercase tracking-tighter">{t.voicesTitle}</h2>
          
          {publicTestimonials.length > 0 ? (
            <div className="relative w-full max-w-4xl mx-auto mb-10 md:mb-16">
              <div 
                ...
                onPointerUp={(e) => {
                  ...
                  if (distance > 50) setCurrentTestimonial(prev => (prev + 1) % publicTestimonials.length);
                  if (distance < -50) setCurrentTestimonial(prev => prev === 0 ? publicTestimonials.length - 1 : prev - 1);
              ...
              >
                ...
                >
                  {publicTestimonials.map((testimonio: any) => (
                    <div key={testimonio.id} className="min-w-full px-2 md:px-4">
                      <div className="bg-white/[0.02] p-6 md:p-10 rounded-3xl border border-white/5 backdrop-blur-sm mx-auto shadow-2xl select-none">
                        <div className="flex justify-center text-sierra-gold mb-4 md:mb-6 text-lg md:text-xl">{[...Array(testimonio.rating || 5)].map((_, i) => <span key={i}>★</span>)}</div>
                        <p className="text-lg md:text-3xl text-white/90 italic font-light mb-6 md:mb-8 leading-relaxed">"{testimonio.comment}"</p>
                        <div className="flex flex-col items-center">
                          <span className="text-sm md:text-base font-bold uppercase tracking-widest text-white">{testimonio.name || 'Anónimo'}</span>
                          <span className="text-[10px] md:text-xs text-white/40 font-bold uppercase tracking-widest mt-2">
                            {testimonio?.created_at ? new Date(testimonio.created_at).toLocaleDateString() : ''}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex justify-center gap-2 md:gap-3 mt-8 md:mt-10">
                {publicTestimonials.map((_, idx) => (
                  <button 
                    ...
                    className={`h-1.5 md:h-2 rounded-full transition-all duration-300 ${currentTestimonial === idx ? 'w-8 md:w-10 bg-sierra-gold' : 'w-2 md:w-3 bg-white/20 hover:bg-white/40'}`}
                  />
                ))}
              </div>
            </div>
          ) : (
            <div className="max-w-3xl mx-auto text-white/40 italic mb-12">
              {lang === 'es' ? 'Sé el primero en compartir tu experiencia con nosotros.' : 'Be the first to share your experience with us.'}
            </div>
          )}
          
          <button onClick={() => setShowReviewModal(true)} className="px-8 py-3 md:px-10 md:py-4 border border-sierra-gold/40 text-sierra-gold hover:bg-sierra-gold hover:text-black transition-all duration-300 rounded-full uppercase tracking-widest text-[10px] md:text-xs font-bold shadow-lg">{t.shareReviewBtn}</button>
        </div>
      </section>

      <footer className="bg-[#0B1320] border-t border-white/5 pt-12 md:pt-20 pb-8 md:pb-10 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-12 mb-12 md:mb-16 relative">
            ...
            <div>
              <h4 className="text-xs md:text-sm font-bold uppercase tracking-[0.2em] text-sierra-gold mb-4 md:mb-6">{t.hoursTitle}</h4>
              <ul className="space-y-4 md:space-y-5 text-sm md:text-base font-light text-white/80">
            ...
            <div>
              <h4 className="text-xs md:text-sm font-bold uppercase tracking-[0.2em] text-sierra-gold mb-4 md:mb-6">{t.followUs}</h4>
              <div className="flex gap-4 mb-6 md:mb-8">
                {/* Instagram Icon */}
                <a href="https://www.instagram.com/sierraapexgroup/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/20 flex items-center justify-center hover:border-sierra-gold hover:text-sierra-gold cursor-pointer transition-all text-white decoration-transparent">
                ...
                {/* Facebook Icon */}
                <div onClick={() => alert(lang === 'es' ? 'Estamos trabajando en nuestras redes sociales. ¡Regresa más tarde!' : 'We are working on our social networks. Please check back later!')} className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/20 flex items-center justify-center hover:border-sierra-gold hover:text-sierra-gold cursor-pointer transition-all text-white">
          ...
          <div className="border-t border-white/10 pt-6 md:pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
            <p className="text-[10px] md:text-xs uppercase tracking-widest text-white/40 font-bold">{t.rights}</p>
            <div className="flex flex-wrap justify-center gap-4 md:gap-6 text-[10px] md:text-xs uppercase tracking-widest text-white/40 font-bold">
      ...
      {/* --- MODAL DE OPINIONES (ENVIAR) --- */}
      {showReviewModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="bg-[#162439] border border-white/10 p-6 md:p-10 rounded-3xl max-w-md w-full relative animate-in fade-in zoom-in-95 duration-300">
            <button onClick={() => setShowReviewModal(false)} className="absolute top-4 right-4 md:top-6 md:right-6 text-white/40 hover:text-white text-xl">✕</button>
            <h3 className="text-2xl md:text-3xl font-bold mb-2 md:mb-3 uppercase tracking-tighter text-sierra-gold">Su Opinión</h3>
            <p className="text-xs md:text-sm text-white/60 mb-6 md:mb-8">La reseña será publicada tras la aprobación del administrador.</p>
            <div className="space-y-4 md:space-y-5">
              <input type="text" placeholder="Nombre (Anónimo opcional)" value={reviewForm.name} onChange={(e) => setReviewForm({...reviewForm, name: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-4 text-sm md:text-base text-white outline-none focus:border-sierra-gold transition-all" />
              <div className="flex gap-2 md:gap-3 justify-center py-2 md:py-3">
                {[1,2,3,4,5].map((star: number) => (
                  <button key={star} type="button" onClick={() => setReviewForm({...reviewForm, rating: star})} className={`text-3xl md:text-4xl transition-colors outline-none ${reviewForm.rating >= star ? 'text-sierra-gold drop-shadow-[0_0_8px_rgba(212,175,55,0.5)]' : 'text-white/20 hover:text-sierra-gold/50'}`}>★</button>
                ))}
              </div>
              <textarea rows={4} placeholder="Experiencia..." value={reviewForm.comment} onChange={(e) => setReviewForm({...reviewForm, comment: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-4 text-sm md:text-base text-white outline-none focus:border-sierra-gold transition-all resize-none"></textarea>
              <button onClick={handleReviewSubmit} disabled={isSubmittingReview} className={`w-full py-4 md:py-5 font-bold rounded-xl uppercase text-[10px] md:text-xs tracking-widest shadow-lg transition-all ${isSubmittingReview ? 'bg-sierra-gold/50 text-black/50 cursor-wait' : 'bg-sierra-gold text-black hover:bg-white'}`}>
                {isSubmittingReview ? 'Enviando...' : 'Enviar para aprobación'}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="w-full md:w-3/5 h-[45vh] md:h-auto min-h-[300px] relative flex flex-col bg-black group shrink-0">
              <div className="flex-1 relative w-full h-full">
                ...
                {selectedCar?.status === 'Vendido' && (
                  <div className="absolute inset-0 bg-black/40 z-20 flex items-center justify-center pointer-events-none backdrop-blur-[2px]">
                    <span className="border-2 md:border-4 border-red-500/80 text-red-500/80 px-4 md:px-6 py-1 md:py-2 text-2xl md:text-5xl font-black uppercase tracking-[0.4em] -rotate-12 shadow-2xl">{t.soldBadge}</span>
                  </div>
                )}

                {selectedCar?.images && selectedCar.images.length > 1 && typeof selectedCar.images !== 'string' && (
                  <>
                    <button onClick={(e) => { e.stopPropagation(); setActivePublicImageIndex(prev => prev > 0 ? prev - 1 : selectedCar.images.length - 1); }} className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-12 md:h-12 bg-black/30 hover:bg-black/80 text-white/60 hover:text-white rounded-full flex items-center justify-center backdrop-blur-md transition-all z-10 opacity-100 md:opacity-0 group-hover:opacity-100">
                      <svg className="w-5 h-5 md:w-7 md:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"></path></svg>
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); setActivePublicImageIndex(prev => prev < selectedCar.images.length - 1 ? prev + 1 : 0); }} className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-12 md:h-12 bg-black/30 hover:bg-black/80 text-white/60 hover:text-white rounded-full flex items-center justify-center backdrop-blur-md transition-all z-10 opacity-100 md:opacity-0 group-hover:opacity-100">
                      <svg className="w-5 h-5 md:w-7 md:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
                    </button>
                  </>
                )}
              </div>
              ...
              {selectedCar?.images && selectedCar.images.length > 1 && typeof selectedCar.images !== 'string' && (
                <div className="h-20 md:h-28 bg-[#0B1320] flex gap-2 md:gap-3 overflow-x-auto p-2 md:p-4 snap-x hide-scrollbar border-t border-white/5 shrink-0 z-10 relative">
                  {selectedCar.images.map((url: any, idx: number) => (
                    <img key={idx} src={url} onClick={() => setActivePublicImageIndex(idx)} className={`h-full w-20 md:w-28 object-cover cursor-pointer rounded-lg shrink-0 snap-start border-2 transition-all ${activePublicImageIndex === idx ? 'border-sierra-gold opacity-100' : 'border-transparent opacity-40 hover:opacity-80'}`} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                  ))}
                </div>
              )}
            </div>
                ...
                <div className="animate-in fade-in slide-in-from-right-4 mt-2 md:mt-0">
                  ...
                  {selectedCar?.status === 'Vendido' ? (
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setShowLeadForm(true)} className="flex-1 py-4 md:py-5 bg-sierra-gold text-black font-bold rounded-xl hover:bg-white transition-all uppercase tracking-widest text-[10px] md:text-sm shadow-[0_0_20px_rgba(212,175,55,0.3)]">{t.reqDetailsBtn}</button>
                      <a href={`https://wa.me/17868506179?text=${encodeURIComponent(`${t.waText} ${selectedCar?.year || ''} ${selectedCar?.brand || ''} ${selectedCar?.model || ''}.`)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center px-4 md:px-5 py-4 md:py-5 bg-sierra-gold text-black font-bold rounded-xl hover:bg-white transition-all shadow-[0_0_20px_rgba(212,175,55,0.3)]">
                      ...
                      <a href="tel:+17868506179" className="flex items-center justify-center px-4 md:px-5 py-4 md:py-5 bg-sierra-gold text-black font-bold rounded-xl hover:bg-white transition-all shadow-[0_0_20px_rgba(212,175,55,0.3)]">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 shrink-0"><path fillRule="evenodd" d="M1.5 4.5a3 3 0 013-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 01-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 006.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 011.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 01-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5z" clipRule="evenodd"/></svg>
                      </a>
                    </div>
                  )}
                </div>
              ) : (
                <div className="animate-in fade-in slide-in-from-left-4 mt-2 md:mt-0">
                  <button onClick={() => setShowLeadForm(false)} className="text-sierra-gold text-[10px] md:text-xs uppercase font-bold tracking-[0.2em] mb-6 md:mb-8 hover:text-white">{t.goBack}</button>
                  <h3 className="text-2xl md:text-3xl font-bold text-white mb-4 md:mb-6">{t.interestIn} {selectedCar?.brand || 'Auto'}</h3>
                  <div className="space-y-4 md:space-y-5">
                    <input type="text" placeholder={t.yourName} value={leadForm.name} onChange={(e) => setLeadForm({...leadForm, name: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-4 text-sm md:text-base text-white outline-none focus:border-sierra-gold transition-colors" />
                    <div className="flex flex-col md:flex-row gap-4 md:gap-5">
                      <input type="email" placeholder={t.email} value={leadForm.email} onChange={(e) => setLeadForm({...leadForm, email: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-4 text-sm md:text-base text-white outline-none focus:border-sierra-gold transition-colors" />
                      <input type="tel" placeholder={t.phone} value={leadForm.phone} onChange={(e) => setLeadForm({...leadForm, phone: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-4 text-sm md:text-base text-white outline-none focus:border-sierra-gold transition-colors" />
                    </div>
                    <textarea rows={4} placeholder={t.specialReq} value={leadForm.message} onChange={(e) => setLeadForm({...leadForm, message: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-4 text-sm md:text-base text-white outline-none focus:border-sierra-gold resize-none transition-colors" />
                    <button onClick={handleLeadSubmit} disabled={isSubmittingLead} className={`w-full py-4 md:py-5 font-bold rounded-xl uppercase tracking-widest text-[10px] md:text-sm transition-all ${isSubmittingLead ? 'bg-sierra-gold/50 text-black/50 cursor-wait' : 'bg-sierra-gold text-black hover:bg-white shadow-[0_0_20px_rgba(212,175,55,0.3)]'}`}>{isSubmittingLead ? t.sending : t.confirmInterest}</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {/* CHATBOT */}
      <button onClick={() => setIsChatOpen(true)} className={`fixed bottom-4 right-4 md:bottom-8 md:right-8 z-[90] w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center transition-all duration-500 hover:scale-110 ${isChatOpen ? 'scale-0 opacity-0 pointer-events-none' : 'scale-100 opacity-100'} ${isScrolled ? 'bg-sierra-gold shadow-2xl border-none' : 'bg-[#111C2D]/60 backdrop-blur-md border border-white/10 shadow-lg hover:bg-sierra-gold hover:border-sierra-gold'}`}>
        <img src="/logo-vertical.svg" alt="Asistente Sierra Apex" className="w-8 h-8 md:w-10 md:h-10 object-contain" onError={(e) => { e.currentTarget.style.display = 'none'; document.getElementById('fallback-chat-icon')!.style.display = 'block'; }} />
        <svg id="fallback-chat-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-6 h-6 md:w-8 md:h-8 hidden ${isScrolled ? 'text-black' : 'text-sierra-gold'}`}><path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
      </button>

      {/* VENTANA CHAT */}
      {isChatOpen && (
        <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-[100] w-[calc(100vw-2rem)] md:w-[380px] max-h-[70vh] md:max-h-none bg-[#162439] border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-10 duration-300">
          
          <div className="bg-[#111C2D] px-4 md:px-6 py-3 md:py-5 flex justify-between items-center border-b border-white/5 shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-sierra-gold animate-pulse"></div>
              <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-sierra-gold">Asistente AI</span>
            </div>
            <button onClick={() => setIsChatOpen(false)} className="text-white/40 hover:text-white transition-colors text-lg md:text-xl p-2 -mr-1">✕</button>
          </div>

          <div className="p-4 md:p-6 flex-1 overflow-y-auto min-h-[200px] max-h-[40vh] md:max-h-[350px] flex flex-col gap-4">
            {chatMessages.map((msg, idx) => (
              <div key={idx} className={`rounded-2xl p-4 md:p-5 max-w-[90%] md:max-w-[85%] border shadow-md ${msg.role === 'bot' ? 'bg-white/5 border-white/5 rounded-tl-none self-start' : 'bg-sierra-gold/20 border-sierra-gold/30 rounded-tr-none self-end text-right'}`}>
                <p className="text-sm md:text-base font-light text-white/90 leading-relaxed">{msg.text}</p>
              </div>
            ))}
          </div>

          <div className="p-3 md:p-5 border-t border-white/10 bg-[#111C2D] shrink-0">
            <div className="relative mb-3">
              ...
              <button onClick={handleSendChatMessage} className="absolute right-1.5 md:right-2 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 bg-sierra-gold rounded-full flex items-center justify-center text-black hover:bg-white transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 md:w-5 md:h-5 ml-0.5"><path d="M3.478 2.405a.75.75 0 00-.926.94l2.432 7.905H13.5a.75.75 0 010 1.5H4.984l-2.432 7.905a.75.75 0 00.926.94 60.519 60.519 0 0018.445-8.986.75.75 0 000-1.218A60.517 60.517 0 003.478 2.405z" /></svg>
              </button>
            </div>
            <a href="https://wa.me/17868506179" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-2.5 bg-[#25D366]/10 border border-[#25D366]/30 hover:bg-[#25D366] text-[#25D366] hover:text-white rounded-full text-xs font-bold uppercase tracking-widest transition-all">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 shrink-0"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              {t.chatWhatsApp}
            </a>
          </div>
      ...
      {showLegal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
          <div className="bg-[#162439] border border-white/10 p-6 md:p-12 rounded-3xl max-w-3xl w-full max-h-[85vh] overflow-y-auto relative animate-in fade-in zoom-in-95 duration-300 shadow-2xl text-white">
            <button onClick={() => setShowLegal(null)} className="absolute top-4 right-4 md:top-6 md:right-6 text-white/40 hover:text-white text-xl md:text-2xl font-bold p-2">✕</button>
            
            {showLegal === 'terms' ? (
              <div className="space-y-4 md:space-y-6 text-xs md:text-base font-light leading-relaxed pr-4 md:pr-0 mt-4 md:mt-0">
                <h2 className="text-xl md:text-2xl font-bold text-sierra-gold uppercase tracking-tighter mb-2 md:mb-4 pr-6">
                  {lang === 'es' ? 'Términos de Servicio' : 'Terms of Service'}
                </h2>
                
                <div className="bg-red-500/10 border-l-4 border-sierra-gold p-3 md:p-4 mb-4 md:mb-6">
                  <h3 className="font-bold text-sierra-gold uppercase mb-1 md:mb-2 text-xs md:text-sm">
            ...
            <button onClick={() => setShowLegal(null)} className="mt-8 md:mt-10 w-full py-3 md:py-4 bg-sierra-gold text-black font-bold rounded-xl uppercase text-[10px] md:text-xs tracking-widest hover:bg-white transition-all shadow-lg">
